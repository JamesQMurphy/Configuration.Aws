...
Validates the build against a compare branch (typically main).  Evaluates the following:
 ...
 4.  If the branch is of the pattern releases/v[SemVer], confirm that the version number 
     matches.  Otherwise, fail.

     For all other branches, if this branch has a SemVer version number less than
     the compare branch, fail the build. Equal is okay.

Based on https://www.jamesqmurphy.com/blog/brewing-the-blog-6 
 
#>

param (
    [String] $CompareBranch = 'main',
